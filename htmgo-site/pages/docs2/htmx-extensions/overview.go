...
func Overview(ctx *h.RequestContext) *h.Page {
	return DocPage(
		...
		h.Div(
			...
			`),
			Link("Trigger Children", "/docs2/htmx-extensions/trigger-children"),
			Link("Mutation Error", "/docs2/htmx-extensions/mutation-error"),
			...
			NextStep(
				"mt-4",
				PrevBlock("Pushing Data", "/docs2/pushing-data/sse"),
				NextBlock("Trigger Children", "/docs2/htmx-extensions/trigger-children"),
			),
		),
	)
}
