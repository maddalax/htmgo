	...
	return DocPage(
		...
		h.Div(
			...
			h.P(
				...
				Link("Caching Components Per Key", "/docs2/performance/caching-per-key"),
				h.Text("."),
			),
			Text(`<b>Usage:</b>`),
			...
			NextStep(
				"mt-4",
				PrevBlock("Swapping", "/docs2/interactivity/swapping"),
				NextBlock("Caching Per Key", "/docs2/performance/caching-per-key"),
			),
		),
	)
}
